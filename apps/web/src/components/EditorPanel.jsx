...
export default function EditorPanel({
  ...
  function handleEditorMount(editor, monaco) {
    ...
    monaco.editor.defineTheme("codeflow-midnight", {
      base: "vs-dark",
      inherit: true,
      rules: [
        { token: "comment", foreground: "66728d", fontStyle: "italic" },
        { token: "keyword", foreground: "a78bfa" },
        { token: "string", foreground: "7ee0b0" },
    ...
    monaco.editor.setTheme("codeflow-midnight");
    updateLineHighlight(currentLine);
  }

  useEffect(() => {
    updateLineHighlight(currentLine);
  }, [currentLine, language.id]);

  let filebarNote = "Curated preview";

  if (executionMode === "live") {
    filebarNote = "Verified execution";
  } else if (executionMode === "ready") {
    filebarNote = isEdited ? "Ready to run" : `${language.label} ready`;
  } else if (isEdited) {
    filebarNote = "Modified sample";
  }
    ...
    <section className="workspace-panel editor-panel">
      <div className="panel-heading">
        ...
        <div className="panel-heading-actions">
          ...
          <span className="editor-language-badge">{language.shortLabel}</span>
        </div>
      </div>
      ...
      <div className="editor-filebar">
        <div className="editor-filetab">
          ...
          {isEdited && (
            <span className="unsaved-indicator" aria-label="File modified" />
          )}
        </div>

        <span className="filebar-note">{filebarNote}</span>
      </div>
      ...
      <div className="editor-statusbar">
        <span>
          {currentLine ? `Current line ${currentLine}` : "No active execution line"}
        </span>
        <span>UTF-8</span>
