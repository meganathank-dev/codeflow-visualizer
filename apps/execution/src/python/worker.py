    ...
    if isinstance(value, (list, tuple, dict, set)):
        ...
        try:
            if isinstance(value, list):
                return [
                    json_safe(item, depth + 1, ancestors)
                    for item in value
                ]
...
def validate_source_policy(tree):
    for node in ast.walk(tree):
        ...
        if isinstance(node, ast.Name):
            if (
                node.id in FORBIDDEN_NAMES
                or node.id.startswith("__codeflow_")
            ):
                raise SourcePolicyError(
                    f'Access to "{node.id}" is not permitted.',
                    line,
                )
...
class PythonExecutionTracer:
    ...
    def record(
        ...
        if len(self.events) >= self.maximum_events - 2:
            raise TraceLimitError(
                (
                    "Python execution exceeded the maximum of "
                    f"{self.maximum_events} events."
                )
            )
    ...
    def update_variable(
        ...
        if (
            previous["valueType"] == "array"
            and current["valueType"] == "array"
        ):
    ...
    def flush_variable_changes(
        self,
        frame,
        line,
    ):
        frame_state = self.frame_states.get(
            id(frame)
        )
        ...
        for name, current_variable in current_locals.items():
            previous_variable = previous_locals.get(
                name
            )
    ...
    def trace_function(
        ...
        if event == "call":
            ...
            if function_name != "<module>":
                ...
                scope_id = (
                    f"{function_name}:{self.frame_number}"
                )
            ...
            if function_name != "<module>":
                ...
                self.record(
                    ...
                    caller_line,
                    {
                        ...
                        "arguments": [
                            item["value"]
                            for item in current_locals.values()
                        ],
                    },
                    (
                        self.frame_states
    ...
    def record_loop_iteration(
        ...
        iteration = (
            self.loop_iterations.get(line, 0)
            + 1
        )
    ...
    def end_loop(
        ...
        iterations = self.loop_iterations.pop(
            line,
            0,
        )
    ...
    def traced_print(
        ...
        **kwargs,
        ...
        if kwargs:
            raise ValueError(
                (
                    "print() file and flush arguments "
                    "are not supported."
                )
            )

        rendered = str(sep).join(
            str(value)
            for value in values
        )
        ...
        if (
            ...
            raise OutputLimitError(
                (
                    "Python execution exceeded the maximum "
                    "console output size."
                )
            )
    ...
    def execution_globals(self):
        return {
            ...
            "__codeflow_while_condition__": (
                self.trace_while_condition
            ),
            "__codeflow_array_access__": (
                self.trace_array_access
            ),
            "__codeflow_method__": (
                self.trace_method
            ),
    ...
    def run(
        ...
        instrumented = SourceInstrumenter(
            source
        ).visit(tree)
...
if __name__ == "__main__":
    try:
        ...
        sys.stderr.write(
            (
                "Python execution worker failed: "
                f"{error}\n"
            )
        )
